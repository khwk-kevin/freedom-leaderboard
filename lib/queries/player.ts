...
export async function getTotalNFTs(fdvId: number): Promise<number> {
  const rows = await neonSql`
    SELECT COALESCE(SUM(CASE WHEN received_nft ~ '^[0-9]+$' THEN received_nft::int ELSE 0 END), 0)::int as total_nfts
    FROM web_card_game_prod.game_match_resulted WHERE player_fdv_id = ${fdvId}
  `;
    ...
    WITH player_agg AS (
      SELECT
        ...
        CASE WHEN COUNT(*) > 0
          THEN (COALESCE(SUM(monster_killed_total), 0)::numeric / COUNT(*)::numeric)
          ELSE 0 END as avg_kills,
        COALESCE(SUM(CASE WHEN received_nft ~ '^[0-9]+$' THEN received_nft::int ELSE 0 END), 0) as total_nfts
      FROM web_card_game_prod.game_match_resulted
      GROUP BY player_fdv_id
      HAVING COUNT(*) >= 10
    ),
    ...
    WITH player_stats AS (
      SELECT
        ...
        CASE WHEN COUNT(*) > 0
          THEN (COALESCE(SUM(monster_killed_total), 0)::numeric / COUNT(*)::numeric)
          ELSE 0 END as avg_kills,
        COALESCE(SUM(CASE WHEN received_nft ~ '^[0-9]+$' THEN received_nft::int ELSE 0 END), 0)::numeric as total_nfts,
        GREATEST(
    ...
    WITH all_ratings AS (
      SELECT
        ...
        ROUND(
          ...
          (LEAST(COALESCE(SUM(CASE WHEN received_nft ~ '^[0-9]+$' THEN received_nft::int ELSE 0 END), 0)::numeric / 50.0 * 100, 100) * 0.10) +
          (LEAST(
            (CASE WHEN COUNT(*) FILTER (WHERE is_nexus = true) > 0
              THEN (COUNT(*) FILTER (WHERE is_nexus = true AND match_result = 'Win')::numeric / COUNT(*) FILTER (WHERE is_nexus = true)::numeric * 100)
              ELSE 0 END * 0.5) +
